...
/* Lab6 content */
extern struct task_struct *current_task;    // proc.c
extern struct task_struct *task[NR_TASKS];  // proc.c
...
uint64 sys_clone(struct pt_regs *regs) {
    ...
    memcpy(task_child, current_task, PGSIZE);
    task_child->thread.ra = (uint64)__ret_from_fork;

    int i;
    for(i = 1; i < NR_TASKS; i++)
        if(task[i] == NULL) break;
    task_child->pid = i;
    task[i] = task_child;
    ...
    memset(task_child->pgd, 0, PGSIZE);
    ...
    memcpy((char *)task_child_user_stack, (char *)PGROUNDDOWN(regs->sscratch), PGSIZE);
    create_mapping(task_child->pgd, USER_END-PGSIZE, task_child_user_stack - PA2VA_OFFSET, PGSIZE, PTE_USER | PTE_WRITE | PTE_READ | PTE_VALID);


    /* 5. 根据 parent task 的页表和 vma 来分配并拷贝 child task 在用户态会用到的内存 */
    for(i = 0; i < current_task->vma_cnt; i++){
        struct vm_area_struct vma = current_task->vmas[i];
        uint64 vm_addr_curr = vma.vm_start;
        while(vm_addr_curr < vma.vm_end) {
            uint64 vm_addr_pg = PGROUNDDOWN(vm_addr_curr);
            uint64 vm_ofs = vm_addr_curr - vm_addr_pg;
            /* 映射存在 */
            if (get_mapping(current_task->pgd, vm_addr_curr)) {
                uint64 pg_copy = kalloc();

                memcpy((char *) pg_copy, (char *) vm_addr_curr, PGSIZE - vm_ofs);
