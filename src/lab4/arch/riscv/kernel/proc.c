...
void uapp_map(struct task_struct *task)
{
    ...
    for (unsigned int i = 0; i < ehdr->e_phnum; i++)
    {
        ...
        if (phdr_curr->p_type == PT_LOAD){
            ...
            uint64 pages_src_addr    = ehdr + phdr_curr->p_offset;
            // p_offset：段内容的开始位置相对于文件开头的偏移量
            memcpy(pages_dest_addr, pages_src_addr, vaddr_round + phdr_curr->p_memsz);
            ...
            create_mapping((uint64 *)task->pgd, PGROUNDDOWN(phdr_curr->p_vaddr), 
...
void user_stack_set(struct task_struct* task){
    ...
    create_mapping((uint64*)task->pgd, USER_END - PGSIZE, user_stack_addr - PA2VA_OFFSET, 1,
